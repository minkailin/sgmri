...
! upper and lower boundary conditions 
  ...
  if(eos.eq.'uns') then 
     ...
     do i=1, nz, nz-1
        ...
        if(vbc .eq. 'flat') then ! no vertical gradient in vx and vy. vz=0
...
!              if(prin .eqv. .true.) write(6,fmt='(A,I3,2(e22.15))') 'REJECT ',i,condition,rates(i)
              ...
              if((prin .eqv. .true.).and.(rates(i).gt.1d-2)) write(6,fmt='(A,I3,2(e22.15))') 'accept ',i,condition, rates(i)
           endif
