...
pro get_stress, loc=loc, nz=nz, nvar=nvar, kmode= kmode 
...
;calculate ente stress

  surf = int_tabulated(zaxis, rho)

  temp = real_part(vx*conj(vy))*rho
  reynolds = int_tabulated(zaxis, temp)/surf
  
  temp = real_part(bx*conj(by))*vasq*rho
  maxwell = int_tabulated(zaxis, temp)/surf

end

pro compare_stress, loc=loc, nz=nz, xrange=xrange, legend=legend, label=label, title=title, yrange=yrange, var=var, nvar=nvar
  common stress, maxwell, reynolds
  ...
  for i=0, nlines-1 do begin
     ...
     get_stress, loc=location(0), nz=nz, nvar=nvar, kmode=ip1

     stress(i) = reynolds - maxwell ;abs(maxwell)/abs(reynolds)
...
; ytitle = '-<Re(\deltav_{Ax}\deltav_{Ay}^{*})>/<Re(\deltav_x\deltav_y^{*})>'
  ...
  plot, xaxis, stress, xmargin=[8,2],ymargin=[3.5,0.5]  $
        ...
        linestyle = 0, xtickinterval=xtickinterval, ytitle=textoidl('R_{xy} - M_{xy}'), charthick=6, yrange=yrange, ystyle=1, xstyle=1 
  for i=1, ncases-1 do begin
     ...
     for j=0, nlines-1 do begin
        ...
        get_stress, loc=location(i), nz=nz, nvar=nvar, kmode=ip1

       stress(j) = reynolds - maxwell
