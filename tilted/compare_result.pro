...
pro compare_result, loc=loc, kmode=kmode, nz=nz, xrange=xrange, legend=legend, label=label
...
;focus on a specific mode (among diff kx, bigQ or beta) 
     ...
     file = filepath('var_axis.dat',root_dir='.',subdir=[location])
     nlines = file_lines(file)
     array = dblarr(2, nlines)
     openr,1,file
     readf,1,array,format='(2(e22.15,x))'
     close,1
     print, 'cases, var', kmode(k), array(1,kmode(k)-1)
...
;  bz = -kx*fq*vx/sig 
  ...
  file = filepath('compare_result.ps',root_dir='.',subdir=[location])
  device, filename=file, $
          bits_per_pixel=8,xsize=8, ysize=4.5,xoffset=0,yoffset=0,/inches
  plot, zaxis, en1(0,*), xmargin=[6,2],ymargin=[3.,1.75], ystyle=0  $
        ,charsize=1.5, thick=4, xrange=xrange, yrange=[0,1], xtitle=textoidl('z/H'), $
        linestyle = 0, xtickinterval=xtickinterval, title=title, charthick=6, ytitle=textoidl('E_m/max(E_m)')
  temp = min(abs(elsasser(0,*)), grid)
  zgrid = zaxis(grid)
  oplot, [zgrid, zgrid], [0,1]*1d10, linestyle=0, thick=1

  for k=1, ncases-1 do begin
     oplot, zaxis, en1(k,*), thick=4, linestyle=k
     temp = min(abs(elsasser(k,*)), grid)
     zgrid = zaxis(grid)
     oplot, [zgrid, zgrid], [0,1]*1d10, linestyle=k, thick=1
  endfor
