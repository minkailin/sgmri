...
!upper disk boundary conditions
  ...
  if((eos.eq.'pol').or.(eos.eq.'iso')) then
     ...
     if((vbc.eq.'wall').or.(vbc.eq.'free')) then
        Dhat = D1til
        !bx = 0
...
!!!below rigid boundary
      if(vbc.eq.'wall') then !dw/dz + dphi/dz = 0 => vz=0

        L33(i,:) = Tp(i,:)
        L34(i,:) = Tp(i,:)

!!!below free boundary. lagrangian gas pressure pert = 0
      else if(vbc.eq.'free') then

        L33(i,:) = sig**2d0*T(i,:) + csq(i)*fQ**2d0*dden*Tp(i,:)
        L34(i,:) = csq(i)*fQ**2d0*dden*Tp(i,:)
     ...
     if(vbc.eq.'nodv') then!      !dv=0 (all components)
       ...
       L34(i,:) = Tp(i,:) 
        ...
        !by=0 condition
        L21(i,:) = (ii*shear)*( fvasq*Tp(i,:) + isig*D1til) + 0.5*kappa2*D1til*sig
        L22(i,:) = fvasq*Tp(i,:)*sig - 2d0*(ii*shear)*D1til + isig*D1til*sig
        L23(i,:) = ifkx*(ii*shear)*D1til
        L24(i,:) = L23(i,:) 
        
        !bz - ii*bx = 0 relation         
        L31(i,:) = fvasq*(Tp(i,:) + kx*T(i,:)) + (D1til + kx*D0til)*isig
        L32(i,:) = (D1til + kx*D0til)*(-2d0)
        L33(i,:) = (D1til + kx*D0til)*ifkx
        L34(i,:) = L33(i,:)
     endif
     
     !GLB65 potential bc. allowing for vertical motion
     L43(i,:) = rho*Tp(i,:)
     L44(i,:) = bigQ*sig**2d0*(Tp(i,:) + kx*T(i,:)) + rho*Tp(i,:)
  endif
