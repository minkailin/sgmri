...
pro get_energies, loc=loc, nz=nz, nvar=nvar, kmode= kmode , chebgrid = chebgrid 
...
pro compare_eigen3, loc=loc, chebaxis=chebaxis, nz=nz, xrange=xrange, legend=legend, nvar=nvar, label=label, title=title, yrange=yrange, var=var, plotrange=plotrange, sample=sample, psym=psym, color=color, solid=solid 
  common energy, emag, epot, ekin, etherm                          
...
;   loadct,color
  ...
  if keyword_set(color) then loadct, color,/silent;,ncolors=255
  for j=0, nsamp-1 do begin
     get_energies, loc=location(i), nz=nz, nvar=nvar(0), kmode=modenum(j), chebgrid=chebgrid
     if not keyword_set(color) then begin
  ...
  loadct, 0,/silent
  oplot, xaxis, data, thick=6, linestyle=solid(i)-1
  endelse
  ...
  for i=1, ncases-1 do begin
     ...
     if keyword_set(color) then loadct, color,/silent;,ncolors=255
     for j=0, nsamp-1 do begin
        get_energies, loc=location(i), nz=nz, nvar=nvar(i), kmode=modenum(j), chebgrid=chebgrid
        if not keyword_set(color) then begin
        if (emag gt epot) then begin
           temp=psym(0)
        endif else temp=psym(1)
  ...
  loadct,color,/silent 
;  colorbar, position=[0.91, 0.192, 0.94, 0.95],/vertical,/right,range=plotrange,format='(f3.1)', thick=4, charsize=1.5 ;,title=textoidl('<E_m>/(<E_m>+<E_g>)'), charsize=1.5
  ...
  if keyword_set(legend) then begin
     ...
     if not keyword_set(color) then begin
        ...
        loadct,color,/silent 
        for j=0, 1 do oplot, [x1,x1], [y0,y0]-dy*j, thick=4, psym=8, color=j*256
        loadct, 0, /silent
        for j=0, 1 do  xyouts, x1, y0-dy*j,textoidl(label(j)),charsize=1.5,charthick=6
     endelse
     if (max(solid) ne 0) then begin
        j=2
        oplot, [x0,x1], [y0,y0]-dy*j, thick=6, linestyle=0
        xyouts, x1, y0-dy*j,textoidl(label(j)),charsize=1.5,charthick=6
        j=3
        oplot, [x0,x1], [y0,y0]-dy*j, thick=6, linestyle=2
        xyouts, x1, y0-dy*j,textoidl(label(j)),charsize=1.5,charthick=6
     endif      
...
;     names ='\beta=' + ['8','10','15','30']

     ;; names = '\Lambda_0='+['0.1','0.2','0.3','0.5']
